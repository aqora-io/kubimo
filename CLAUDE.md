...
kubimo is a Kubernetes operator for running [marimo](https://marimo.io) Python notebooks. It defines CRDs (group `kubimo.aqora.io/v1`) for notebook workspaces and runners, a controller that reconciles them into Pods/Services/Ingresses/Jobs, and a per-node agent (CSI driver) that serves each workspace a slot on the node's shared data volume, with S3 as the source of truth for workspace files.
...
- `agent/` — per-node DaemonSet binary: a CSI driver that provisions each workspace an XFS-project-quota'd slot on the node data volume, hydrates it from S3 on publish, flushes it back on unpublish, and serves warm-pool claims.
...
- **Workspace** (`bmow`) — persistent notebook workspace. The Workspace reconciler does not directly provision storage resources: its files live in S3 (`spec.indexer` names the bucket/keyPrefix) and its slot is carved out of the node data volume by the agent when a runner's inline CSI volume is published, so the controller only materializes status (`Ready` means "nothing left to provision"). `spec.storage.max` is the slot's hard quota. `spec.restoreFrom` (`{bucket, keyPrefix, pod.env, secrets}`) seeds a new workspace's tracked files from an indexer S3 archive — works even after the source workspace was deleted, but requires the archive was written with `uploadContent: true` (see `examples/restore.yaml`). `restoreFrom.secrets` (`Values` | `NamesOnly`, default `NamesOnly`) controls whether the archive's secrets come back with values or as empty `.env` placeholders; the mode reaches the agent as the `seedSecrets` volume attribute.
