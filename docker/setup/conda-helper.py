...
def sync(
    ...
    if expected_lockfiles == actual_lockfiles:
        ...
        missing_lockfiles = (
            ...
            for path in (expected_lockfiles - actual_lockfiles)
        )
    ...
    with click.open_file(file) as fd_env:
        data_env = yaml.safe_load(fd_env)
    ...
    if locked:
        ...
        with NamedTemporaryFile(
            ...
            yaml.safe_dump(data_env, fd_env)
            fd_env.flush()
            fd_env.close()
...
def _uv_pip_compile(
    ...
    *,
    ...
    with NamedTemporaryFile(
        ...
        for pip_dependency in sorted(pip_dependencies):
            _ = fd_pip.write(pip_dependency)
            _ = fd_pip.write("\n")
        fd_pip.flush()
        fd_pip.close()
